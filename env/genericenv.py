...
class GenericEnv(ABC):
    ...
    def update_lift_foot(self, weighting: float, sim_step: int):
        if sim_step == 0: # reset at first sim step
            self.lift_foot = False
        for foot in self.sim.feet_body_name:
            foot_frc = self.sim.get_body_contact_force(name=foot)
            if self.time > 5 and foot_frc[2] == 0:
                self.lift_foot = True
    ...
    def get_dr_ranges(self):
        ...
        # slope
        dr_ranges["slope"] = {"ranges": dyn_rand_data["slope"]}
